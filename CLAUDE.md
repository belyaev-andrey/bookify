...
Both listen with plain `@EventListener` rather than `@ApplicationModuleListener`: they only read, so they need neither
the new transaction nor the asynchronous delivery it adds. Neither touches the database during initialization, which
matters because Liquibase migrates after these beans are constructed — `BookCatalogWarmup` is created after the
`EntityManagerFactory` but before the schema exists.
...
- Writes that bypass the services (repository calls, direct SQL, or another app instance, since Caffeine is
  ...
  member.
- Cached `Book`/`Member` instances are detached Hibernate entities shared by every caller. `BorrowingService.borrowBook`
  passes the cached `Member` into a new `Borrowing` only as an association reference, where Hibernate needs nothing
  but its id. Don't mutate a cached instance and save it; reload it through the repository instead, as `disableMember`
  does.
