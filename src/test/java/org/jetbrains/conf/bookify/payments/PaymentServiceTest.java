...
class PaymentServiceTest {
    ...
    void getApplicableRate_returnsEmptyWhenNoRateConfigured() {
        Optional<BookFineRateEntity> result = paymentService.getApplicableRate(BOOK_WITHOUT_RATE, LocalDate.of(2024, 1, 1));
    ...
    // ==================== Tests for handleAssignFine() ====================

    @Test
    void handleAssignFine_completesWithoutErrorWhenRateConfigured() {
        AssignFineEvent event = new AssignFineEvent(BOOK_WITH_RATE_HISTORY, UUID.randomUUID(), 3);

        paymentService.handleAssignFine(event);
    }

    @Test
    void handleAssignFine_completesWithoutErrorWhenNoRateConfigured() {
        AssignFineEvent event = new AssignFineEvent(BOOK_WITHOUT_RATE, UUID.randomUUID(), 3);
