...
class BookCachingTest {
    ...
    void saveBookRefreshesCachedBookAndEvictsList() {
        Book saved = bookService.saveBook(book(null, "Cache Test Book", "9780000000001", true));
        ...
        try {
            ...
            bookService.saveBook(book(bookId, "Renamed Cache Test Book", "9780000000001", true));

            assertThat(cache(BOOKS_CACHE).get(bookId, Book.class))
                    .isNotNull()
                    .extracting(Book::getName)
                    .isEqualTo("Renamed Cache Test Book");
    ...
    void returnListenerEvictsCachedBook() {
        Book book = bookRepository.save(book(null, "Cache Test Book", "9780000000002", false));
    ...
    private static Book book(@Nullable UUID id, String name, String isbn, boolean available) {
        Book book = new Book();
        book.setId(id);
        book.setName(name);
        book.setIsbn(isbn);
